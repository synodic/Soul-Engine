...
//#define terminator static_cast<std::uint32_t>(-1)
//
...
//
//__device__ __inline__ std::int32_t __float_as_int(float in) {
//	union fi { std::int32_t i; float f; } conv;
//	conv.f = in;
//	return conv.i;
//}
//
//__device__ __inline__ float __int_as_float(std::int32_t a)
//
//{
//
//	union { std::int32_t a; float b; } u;
//
//	u.a = a;
//
//	return u.b;
...
//{
//	std::int32_t a2 = __float_as_int(a);
//	std::int32_t b2 = __float_as_int(b);
...
//__device__ __inline__ std::int32_t   min_min(std::int32_t a, std::int32_t b, std::int32_t c) {
//	std::int32_t v;
//	asm("vmin.s32.s32.s32.min %0, %1, %2, %3;"
//		: "=r"(v) : "r"(a), "r"(b), "r"(c));
//	return v;
//}
//__device__ __inline__ std::int32_t   min_max(std::int32_t a, std::int32_t b, std::int32_t c) { std::int32_t v; asm("vmin.s32.s32.s32.max %0, %1, %2, %3;" : "=r"(v) : "r"(a), "r"(b), "r"(c)); return v; }
//__device__ __inline__ std::int32_t   max_min(std::int32_t a, std::int32_t b, std::int32_t c) { std::int32_t v; asm("vmax.s32.s32.s32.min %0, %1, %2, %3;" : "=r"(v) : "r"(a), "r"(b), "r"(c)); return v; }
//__device__ __inline__ std::int32_t   max_max(std::int32_t a, std::int32_t b, std::int32_t c) { std::int32_t v; asm("vmax.s32.s32.s32.max %0, %1, %2, %3;" : "=r"(v) : "r"(a), "r"(b), "r"(c)); return v; }
...
//	const glm::vec3& rayO, std::int32_t kx, std::int32_t ky, std::int32_t kz, float Sx, float Sy, float Sz,
...
//
//	/*std::int32_t det_sign = glm::sign(det);
...
//__device__ std::uint32_t HighestBit(std::uint32_t i, std::int64_t* morton)
//{
//	return morton[i] ^ morton[i + 1];
//}
//
//__global__ void BuildTree(std::uint32_t n, BVH* data, Node* nodes, std::int64_t* mortonCodes, BoundingBox* boxes)
//{
...
//
//	std::uint32_t nodeID;
//	const std::uint32_t innerSize = n - 1;
//
...
//		const std::uint32_t parentID = index;
...
//		const std::uint32_t parentID = index - 1;
//		Node& parentNode = nodes[parentID];
//
...
//
//			// parent = right, set parent left child and range to node
//			const std::uint32_t parentID = node.rangeRight;
//			Node& parentNode = nodes[parentID];
//
...
//
//			// parent = left -1, set parent right child and range to node
//			const std::uint32_t parentID = node.rangeLeft - 1;
//			Node& parentNode = nodes[parentID];
//
...
//}
//
//__global__ void Reset(std::uint32_t n, Node* nodes)
//{
//
//	const std::uint32_t index = ThreadIndex1D();
//
...
//}
//
//__device__ bool BVH::IsLeaf(std::uint32_t test) const {
...
//			std::uint32_t childL = nodes[currentNode].childLeft;
//			std::uint32_t childR = nodes[currentNode].childRight;
//
...
//		{
//			const std::uint32_t faceID = currentLeaf - leafSize;
