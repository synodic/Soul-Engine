...
private:
    ...
    std::uint32_t FindMemoryType(std::uint32_t, const vk::MemoryPropertyFlags&) const;
    ...
    device_(device)
{
	...
	std::array<std::uint32_t, 1> queueFamily = { device_->GetGraphicsIndex() };
...
template <typename T>
std::uint32_t VulkanBuffer<T>::FindMemoryType(const std::uint32_t typeFilter, const vk::MemoryPropertyFlags& properties) const {
