...
//
//inline __device__ __host__ std::uint32_t PowTwoDivider(std::uint32_t n)
//{
//	if (n == 0) return 0;
...
//}
//
//__device__ const glm::vec4 GetClamped(const glm::vec4* data, const glm::ivec2& originalSize, std::int32_t x, std::int32_t y)
//{
...
//
//
//__global__ void GPUHermiteBicubic(std::uint32_t n, glm::vec4* data, glm::vec4* buffer, glm::uvec2 originalSize, glm::uvec2 desiredSize) {
//
...
//	std::int32_t xint = std::int32_t(xOld);
//	float xfract = xOld - glm::floor(xOld);
//
//	float yOld = v * originalSize.y;
//	std::int32_t yint = std::int32_t(yOld);
...
//}
//
//__global__ void GPUNearest(std::uint32_t n, glm::vec4* data, glm::vec4* buffer, const glm::uvec2 originalSize, const glm::uvec2 desiredSize) {
...
//
//	const std::int32_t xInt = std::int32_t(u * desiredSize.x);
//	const std::int32_t yInt = std::int32_t(v * desiredSize.y);
//
//	const std::int32_t oldIndex = yInt * desiredSize.x + xInt;
...
//
//		std::uint32_t count = originalSize.x*originalSize.y;
//
//		std::uint32_t blockSize = 64;
//		std::uint32_t blockCount = (count + blockSize - 1) / blockSize;
//
