...
//};
//
//std::uint32_t getMipMapLevels(cudaExtent size)
//{
...
//	return levels;
//}
//inline void _checkHost(bool test, const char *condition, const char *file, std::int32_t line, const char *func)
//{
//	if (!test)
//	{
...
//
//__global__ void
//d_mipmap(cudaSurfaceObject_t mipOutput, cudaTextureObject_t mipInput, std::uint32_t imageW, std::uint32_t imageH)
//{
...
//		dim3 gridSize(((std::uint32_t)width + blockSize.x - 1) / blockSize.x, ((std::uint32_t)height + blockSize.y - 1) / blockSize.y, 1);
//
//		d_mipmap << <gridSize, blockSize >> >(surfOutput, texInput, (std::uint32_t)width, (std::uint32_t)height);
//
...
//
////rgba
//unsigned char* DirectLoad(const char* fileName, std::int32_t* width, std::int32_t* height, std::int32_t*texChannels){
