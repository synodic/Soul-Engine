...
//
//
//		const std::int32_t index = ThreadIndex1D();
//
//		if (index >= n) {
//			return;
//		}
//
//		const std::int32_t startIndex = 0;
//
//		std::int32_t cur = 0;
...
//		const std::uint32_t sampleIndex = (index - startIndex) / glm::ceil(samples); //the index of the pixel / sample
//		const auto localIndex = (index - startIndex) % static_cast<std::int32_t>(glm::ceil(samples));
//
...
//		Ray ray = rays[index];
//
//		std::uint32_t faceHit = ray.currentHit;
//
//		float hitT = ray.direction.w;
//
//		std::uint32_t localIndex = ray.resultOffset;
//
//		curandState randState = randomState[index];
//
//		glm::vec3 col;
//
//		if (faceHit == std::uint32_t(-1)) {
//
...
//		}
//
//		std::int32_t samples = job[cur].camera.film.hits[localIndex];
//
//		col /= samples;
//
...
//	}
//
//	__global__ void ExecuteJobs(std::uint32_t n, Ray* rays, BVH* bvhP, Vertex* vertices, Face* faces, BoundingBox* boxes, std::int32_t* counter) {
//
//		BVH bvh = *bvhP;
//
//		Ray ray;
//		std::int32_t rayidx;
//
//		extern __shared__ volatile std::int32_t nextRayArray[]; // Current ray index in global buffer needs the (max) block height.
//
//		do {
//			const std::uint32_t tidx = threadIdx.x;
//			volatile std::int32_t& rayBase = nextRayArray[threadIdx.y];
//
//
//			const bool          terminated = bvh.IsTerminated();
//			const std::uint32_t  maskTerminated = __ballot(terminated);
//			const std::int32_t           numTerminated = __popc(maskTerminated);
//			const std::int32_t           idxTerminated = __popc(maskTerminated & ((1u << tidx) - 1));
//
//			//fetch a new ray
//
//			if (terminated)
...
//				ray = rays[rayidx];
//
//				ray.currentHit = static_cast<std::uint32_t>(-1);
